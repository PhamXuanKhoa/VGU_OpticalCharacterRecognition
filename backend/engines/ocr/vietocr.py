...
# --- NLTK Imports and Download ---
try:
    import nltk
    from nltk.corpus import words
    # Download the 'words' corpus on the first run if it's not already present
    try:
        words.words()
    except LookupError:
    ...
    # Load the English vocabulary into a set for fast lookups
...
def clean_ocr_output(text: str) -> str:
    """
    Cleans the raw output from the OCR model by removing common noise patterns.
    Args:
        text (str): The raw multiline text extracted by the OCR.
    Returns:
        str: The cleaned text with noise removed.
    """
    noise_words = {
        "contractionalized", "contransitionalized", "contransitionalism",
        "contransiboriousness", "expectiveness", "convemaquy",
        "communicality", "electronometers", "contractionalists",
        "contransitionalists", "contransmentations", "carcilitation",
        "claticalizations", "contranspectionalized", "contransitionalizations",
        "commonstructionalists", "transman",
        "anticulations"
    }
    header_noise_keywords = {"qhang", "anguia"}
    cleaned_lines = []
    for line in text.splitlines():
        line_upper = line.upper()
        if any(keyword.upper() in line_upper for keyword in header_noise_keywords) and line.isupper():
            continue
        words = line.split()
        clean_words = []
        for word in words:
            # Rule 1: Remove long words that are mostly numeric
            if len(word) > 10:
                num_digits = sum(c.isdigit() for c in word)
                if (num_digits / len(word)) > 0.8:
                    continue
            
            # Clean the word for dictionary lookups
            clean_word = word.lower().strip(".,:;-?!")
            # Rule 2: Remove words from the manual noise blocklist
            if clean_word in noise_words:
                continue
            
            # Rule 3: Remove common English words that are not likely Vietnamese
            if len(clean_word) > 2 and clean_word in ENGLISH_VOCAB:
                continue
...
class VietOCRCroppingEngine(OCREngine):
    def __init__(self):
        ...
        self.horizontal_overlap_percentage = 50 # NEW: Hardcoded horizontal overlap percentage
    ...
    def extract_text(self, image_path: str) -> str:
        print(f"--- [ENGINE: VietOCR Cropping] Processing image at {image_path} ---")
        try:
            ...
            horizontal_overlap = int(self.max_width * (self.horizontal_overlap_percentage / 100)) # NEW: Define here
            ...
            return f"Error with VietOCR cropping engine: {e}"